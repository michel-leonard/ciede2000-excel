...
<details>
<summary>How are the columns organized in the Excel file?</summary>

### A single-line formula

The first L\*a\*b\* color to be compared is defined in columns `A`, `B` and `C`, the second in columns `D`, `E` and `F`. Parametric factors kL, kC and kH are defined in columns `G`, `H` and `I`, while the CIEDE2000 result is displayed in column `J`. A static test to be deleted in production is found in column `K`. The complete formula is stored in the hidden columns from `L` to `AM`. The formula takes special cases into account, e.g. `atan2(0, 0)` is managed in the `S` and `T` columns, to ensure consistent mathematical behavior. The `AN` column is grayed out to make it easier to expand the formula, when you need to calculate more lines than the demo.
